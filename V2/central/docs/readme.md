...
- `openapi.yaml`: fonte canônica OpenAPI 3.1 do contrato HTTP publicado, incluindo a reconciliação OTA administrativa; gere o cliente com `npm run generate:api`.
- `operacao.md`: configuração de ambiente, inicialização, saúde, logs e persistência.
- `implementacao.md`: tarefas isoladas, critérios de aceite e verificações manuais para construir a central; T01–T14 concluídas.
