...
void set_pin_intr(gpios_intr_t* intr)
{
    ...
    {
        gpio_pin_intr_state_set(GPIO_ID_PIN(intr->gpio_id), GPIO_PIN_INTR_POSEDGE);
    }
}
...
void gpios_intr_handler(void *arg)
{
    ...
    for (i = 0; i < gpios_intrs_count; i++)
    {
        ...
        uint8_t new_state = GPIO_INPUT_GET(GPIO_ID_PIN(intr->gpio_id));

        if (new_state != intr->state)
        {
